...
앞서 언급한 고유벡터가 변화에 대한 새로운 축으로 작동한다는 관점은 굉장히 중요하다.

그렇다면 고윳값의 의미는 무엇일까? 고윳값의 부호는 해당 고유벡터 축에서 원점으로 가까워질지 멀어질지를 결정해주는 요소였다고 하면,

고윳값의 크기는 얼마나 빨리 원점으로 가까워질지 멀어질지를 결정해주는 요소다.

한편, [미분방정식과 자연상수 e](https://angeloyeo.github.io/2021/05/05/ODE_and_natural_number_e.html) 편에서 본 것 처럼 우리는 매번 다음번 좌표를 정할 때 마다 현재 결과물을 바탕으로 성장하므로 좌표의 이동은 연속성장을 이용해 표현할 수 있기 때문에 자연상수 $e$를 이용해서 좌표의 변화를 표현할 수 있다.

그리고 [자연상수 위의 승수의 역할이 바로 성장 속도](https://angeloyeo.github.io/2019/09/04/natural_number_e.html)와 관련된 것이므로 고윳값을 그대로 이용해 성장 속도를 표현할 수 있을 것이다.

정리하면, 연립 미분방정식의 일반해는 다음과 같이 서술할 수 있다.

$$\begin{bmatrix}x(t) \\y(t) \end{bmatrix}=c_1 v_1 \exp(\lambda_1 t) +c_2 v_2 \exp(\lambda_2 t) $$

따라서, 식 (6)으로 쓸 수 있는 미분방정식의 해는 다음과 같을 것이다.
