...
1 라디안(radian)은 원둘레 위에서 반지름의 길이와 같은 길이를 갖는 호에 대응하는 중심각의 크기로 무차원의 단위이다. 보다 일반적으로 라디안 값은 원에서의 호와 반지름의 길이의 비율과 같다. 즉, θ = s /r 이다, 여기서 θ 는 라디안으로 주어진 각도, s 는 호의 길이, r 은 반경이다. (출처 : Wikipedia)
 ...
 호도법이 더 논리적이고 수학적인 이유는 원이 가지는 수학적인 특성을 잘 활용했기 때문이다. 원은 (반)지름과 둘레가 항상 일정한 비율을 가지고 있다. 즉, 어떤 원이더라도 반지름과 원의 둘레는 항상 <img height = "auto" src = "https://raw.githubusercontent.com/angeloyeo/angeloyeo.github.io/master/equaions/2019-06-04-2-1-angle_rad/eq1.png">만큼의 비율을 가지고 있게 되고, radian 각도(angle) 시스템은 원의 특성을 잘 반영한 시스템이라고 할 수 있다.
...
$$\frac{1}{2}\times 1 \times \sin(\theta \degree) \leq \pi \times 1^2 \times \frac{\theta\degree}{360\degree}\leq \frac{1}{2}\times 1 \times \tan(\theta\degree)$$

다시 쓰자면,

$$\frac{\sin(\theta\degree)}{2}\leq \pi \frac{\theta\degree}{360\degree}\leq \frac{\tan(\theta\degree)}{2}$$

양변에 360도를 곱하고, $\pi$로 나눠주면,

$$\frac{360\degree}{\pi}\times \frac{\sin(\theta\degree)}{2}\leq \theta\degree\leq \frac{360\degree}{\pi}\times\frac{\tan(\theta\degree)}{2}$$

양변을 $\sin(\theta\degree)$로 나눠주고 식을 정리하면,

$$\frac{180\degree}{\pi}\leq\frac{\theta\degree}{\sin(\theta\degree)}\leq\frac{180\degree}{\pi}\times\frac{1}{\cos(\theta\degree)}$$

분자 분모를 뒤집어주면,

$$\frac{\pi}{180\degree}\geq \frac{\sin(\theta\degree)}{\theta}\geq\frac{\pi}{180\degree}\times \cos(\theta\degree)$$

따라서 $\theta\degree \to 0$으로 수렴할 때, 

$$\frac{\sin(\theta\degree)}{\theta \degree}$$
