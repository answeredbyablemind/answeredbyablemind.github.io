...
$\quad$ 0. 0회 성공부터 10회 성공까지의 카운트 수를 모두 0으로 세팅해둔다.

$\quad$ 1. 10회 동전 던지기를 한다.

$\quad$ 2. 이 중 몇 번 앞면이 나왔는지(성공 횟수)를 세어본다

$\quad$ 3. 해당되는 성공회수의 성공 카운트를 올려준다. 가령 성공 횟수가 3이었다면 3회 성공의 카운트 수는 +1이 되는 것이다.

$\quad$ 4. 1~3을 무수히 많이 반복한다. (가령 100회 반복) 

$\quad$ (물론 반복 회수는 많을 수록 선험적 확률 분포와 더 가까운 값을 얻을 수 있다.)

아래의 영상은 위의 0~4에서 소개한 시뮬레이션을 직접 수행해 카운트를 히스토그램 형태로 그려본 것이다.
