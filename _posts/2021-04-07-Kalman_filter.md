...
확률 분포를 이용해 할 수 있는 일 중 하나는 상태(혹은 동작)에 대한 확률적인 서술이다.

가령 친구와 만나기로한 약속 장소로 서초역 3번 출구를 정했다고 하자. 그럼 이 친구는 서초역 3번 출구의 바로 앞에 있을 가능성이 굉장히 높다.

하지만, 그 주변에서 서성일 수도 있고, 아니면 혹시나 어떤 이유에서든지 다른 곳에 서있을 수도 있지만, 그 확률은 낮고... 와 같은 방식으로 많은 가능성들에 대해서

"확률적으로" 현재 상태를 서술할 수도 있다. 가령, 아래와 같이 표로 정리해볼 수 도 있을 것이다.
