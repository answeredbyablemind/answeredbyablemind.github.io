...
아래 그림과 같이 법선 벡터는 접선 벡터를 $-90\degree$ 회전 시켜줌으로써 얻을 수 있다.
...
$\cos(-90\degree)$는 0이고, $\sin(-90\degree)$는 -1이므로 회전변환행렬은
...
임을 알 수 있다. 즉, 어떤 벡터를 $-90\degree$ 회전시켜주려면 벡터의 두 컴포넌트를 바꿔주고 바뀐 $y$ 컴포넌트에는 -1을 곱해주면 되는 것이다.
