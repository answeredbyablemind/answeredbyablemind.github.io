...
```{Python}
  ...
  S = nn.gradprop(T)**2
  Y = nn.forward(X) # Y는 보기에 softmax에 들어가기 전 cross entropy의 출력값 같은데... 그러면 무조건 softmax로 output을 classify하는 경우에만 가능한걸까?
  # 아니면 sigmoid 넣기 전 값을 가지고 Y 값을 만들면 binary classification에도 가능한걸까?
