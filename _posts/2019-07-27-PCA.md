...
공분산 행렬은 일종의 행렬로써, 데이터의 구조를 설명해주며, 특히 특징 쌍(feature pairs)들이 얼마나 닮았는가(다른 말로는 얼마만큼이나 함께 변하는가)를 행렬에 나타내고 있다.

## 공분산 행렬의 기하학적 의미

일단은 복잡한 수식적인 전개 이전에 공분산 행렬을 기하학적으로 파악해보도록 하자. 행렬이란 선형 변환이고 하나의 벡터 공간을 선형적으로 다른 벡터 공간으로 mapping 하는 기능을 가진다.[^1]
...
고유 벡터(eigenvector)의 의미를 잘 생각해보면[^2], 고유 벡터는 그 행렬이 벡터에 작용하는 주축(principal axis)의 방향을 나타내므로 공분산 행렬의 고유 벡터는 데이터가 어떤 방향으로 분산되어 있는지를 나타내준다고 할 수 있다. 

[^2]: 고유벡터에 대해서는 다음의 내용을 참고. https://angeloyeo.github.io/2019/07/17/eigen_vector.html

고유 값은 고유벡터 방향으로 얼마만큼의 크기로 벡터공간이 늘려지는 지를 얘기한다. 따라서 고유 값이 큰 순서대로 고유 벡터를 정렬하면 결과적으로 중요한 순서대로 주성분을 구하는 것이 된다.

즉, 우리가 고민했던 문제는 무엇인가? 바로 '데이터 벡터를 어떤 벡터에 내적(혹은 정사영)하는 것이 최적의 결과를 내주는가?' 이다. 이 문제에 대한 해결은 바로 공분산 행렬의 고유값과 고유 벡터를 구함으로써 가능하다.
...
이 때, 아래의 행렬의 각 열(feature)들의 평균 값은 $0$이라고 하자. 이러한 가정은 위에서 설명한 '부차적인 문제'인 데이터 분포의 중심을 중심축(pivot)으로 움직이는 벡터를 찾는데 도움을 줄 것이다[^3].

[^3]: 만약 각 열들의 평균 값이 0이 아니라고 하면, 각 열들의 평균값을 빼줌으로써 각 열들의 평균값이 0인 행렬을 구할 수 있을 것이다.


<p align="center">
<img width = "400" src = "https://raw.githubusercontent.com/angeloyeo/angeloyeo.github.io/master/pics/2019-07-27_PCA/mat_x.png">
<br>
그림 5. d개의 n차원 열벡터를 쌓아서 얻은 데이터 행렬 X
...
<br>
그림 6. 공분산 행렬을 계산하기 위해 각 데이터 특징들이 서로 얼마나 닮았는지 계산하는 과정.
</p>
...
그림 6의 마지막 항(term)을 보면 $X^TX$의 $i$행 $j$열의 성분인 $(X^TX)_{ij}$는 $d$개의 feature 중 $i$번째 feature와 $j$번째 feature가 얼마나 닮았는지를 모든 사람으로부터 값을 얻어서 내적 연산을 취함으로써 확인시켜준다. 
...
<br>
그림 7. 공분산 행렬의 계산. 데이터의 샘플 수 만큼 그 크기를 나눠주는 과정.
</p>
...
[^4]: 조금 더 엄밀하게는 n대신 n-1로 나누어주어야 sample의 covariance를 얻을 수 있다. n이 꽤 큰 숫자라면 크게 결과가 달라지지는 않겠지만...
...
## 왜 최대 variance를 갖는 정사영 결과를 찾는 것인가?

데이터의 구조를 제일 잘 반영. ... 좀더 글을 생각해보기로.


## 공분산 행렬의 고유벡터와 최대 variance

혹시나 앞서 설명한 고유 벡터와 고유 값을 이용한 기하학적인 설명이 어려웠다면 이 파트에서 설명할 수식적인 설명이 더 도움이 될지도 모르겠다. 이 파트에서는 왜 고유벡터[^4]에 데이터를 정사영하여 얻은 data의 variance가 최대가 되는지에 대해 설명하고자 한다.

[^4]: 혹은 고유 벡터들이 이루는 열벡터 공간

예를 들어 $d$차원의 데이터를 정사영을 통해 1차원으로 차원 감소한다고 해보자. 이 때 정사영 대상인 임의의 단위 벡터 $\vec{e}$를 생각해보자. $\vec{e}$는 $d\times 1$차원의 벡터이다. 그러면 데이터 행렬 $X\in R^{n\times d}$을 단위벡터 $\vec{e}$에 정사영 해주는 것은 다음과 같이 표현할 수 있다.
...
<center><img src="http://bit.ly/2kQQ8ug"></center>
...
<img src="http://bit.ly/2kQPzR3">

=<img src="http://bit.ly/2kQXpdh">

=<img src="http://bit.ly/2kvi6s9">

=<img src="http://bit.ly/2kQHXy2">
</center>
...
라그랑주 승수법을 이용하도록 하자. 이 때 목표함수는 $\vec{e}^T\Sigma\vec{e}$이며 제약조건은 $||\vec{e}||^2=1$이다.

따라서 다음의 보조방정식을 만들 수 있다.

<center><img src="http://bit.ly/2krowIp"></center>

보조방정식 $L$을 $\vec{e}$에 대해 편미분하면

<center><img src="http://bit.ly/2kro3Gx"></center>

이므로

<center><img src="http://bit.ly/2labpQt"></center>

의 조건을 만족하는 $\vec{e}$가 선택되면 목표함수 $\vec{e}^T\Sigma\vec{e}$가 최대가 될 수 있다는 것을 알 수 있다.

또, 라그랑주 승수법에 의해 나온 결과를 통해

<center><img src="http://bit.ly/2kvgbng"></center>
...
<center><img src="http://bit.ly/2la4AhZ"></center>
...
<center>
![](https://wikidocs.net/images/page/7646/12.png)

그림 10 scree plot의 예시 [^2]
