...
첫번째 버튼부터 네 번째 버튼까지에 해당하는 행렬들은 각각 다음과 같다.[^note]

[^note]: 좀 더 수학적으로는 A라는 공분산행렬을 갖는 다변수 정규분포의 샘플을 얻기 위해서는 A를 Cholesky decomposition 하여 얻게되는 상삼각행렬(혹은 하삼각행렬)을 이용해 선형변환 해주는 것이 더 타당한 설명이다. 다시 말하면, 분산을 표현하는 행렬이 아니라 표준편차를 표현하는 행렬을 이용해 선형변환 하는 것이 수학적으로는 더 타당하다. 여기서는 이해의 편의를 위해 공분산 행렬을 이용하였다.
