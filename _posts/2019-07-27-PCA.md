...
고유 값은 고유벡터 방향으로 얼마만큼의 크기로 벡터공간이 늘려지는 지를 얘기한다[^3]. 따라서 고유 값이 큰 순서대로 고유 벡터를 정렬하면 결과적으로 중요한 순서대로 주성분을 구하는 것이 된다. 아래의 그림 5에서 고유 벡터 두 개를 볼 수 있고 각각의 벡터의 크기가 각 벡터의 고유값을 의미한다.
...
<br>
그림 7. 공분산 행렬을 계산하기 위해 각 데이터 특징들이 서로 얼마나 닮았는지 계산하는 과정.
</p>
