...
고유 값은 고유벡터 방향으로 얼마만큼의 크기로 벡터공간이 늘려지는 지를 얘기한다. 따라서 고유 값이 큰 순서대로 고유 벡터를 정렬하면 결과적으로 중요한 순서대로 주성분을 구하는 것이 된다. 아래의 그림 5에서 고유 벡터 두 개를 볼 수 있고 각각의 벡터의 크기가 각 벡터의 고유값을 의미한다.
...
라그랑주 승수법을 이용하도록 하자. 이 때 목표함수는 $\vec{e}^T\Sigma\vec{e}$이며 제약조건은 $\left|\vec{e}\right|^2=1$이다.
