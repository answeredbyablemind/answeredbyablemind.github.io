...
미분 방정식이 함수값의 변화에 관한 것이라고 하면 식 (2)에서는 어떤 변화를 보여줄까?

그런데, 식 (2)에는 미분 계수 $dy/dx$가 들어있다보니 식이 어려워 보일 수 있다. 미분 계수를 원래의 정의대로 돌려놓고 생각해보자.

다시 말해, $y=f(x)$라고 했을 때,

$$식(2) \Rightarrow \lim_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h}=x % 식 (5)$$
