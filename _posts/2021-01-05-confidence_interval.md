...
$$\text{남자 표본 그룹(6명): }[170, 165, 168, 180, 175, 173]$$

$$\text{여자 표본 그룹(6명): }[165, 162, 158, 170, 163, 152]$$
...
$$t_{0.95} \times SEM = 2.228 * 3.3275 = 7.4137$$

이라는 것을 알 수 있다.

따라서, 남자 여자의 평균키 차이에 대한 95% 신뢰 구간은 

$$(\bar{X}_1 - \bar{X}_2)\pm t_{0.95}\times SEM = 10.16\pm 7.4137$$
...
즉, 누군가 한국 성인 남자들과 성인 여자들의 키 차이가 얼마인지 궁금하다는 질문을 받았다고 했을 때, 위와 같이 6명씩 임의의 사람들을 표본으로 추출해 값을 계산하면 남자가 2.75 cm에서 17.57 cm 정도 크다는 것을 알 수 있다는 의미이다 (95% 확률로 그 사이에 모평균 차이가 들어온다고 확신할 수 있음).
...
[//]:# (문제를 조금 바꿔서 n수가 매우 많은 경우에 대한 문제 뽑을 것... p-value는 매우 작지만 treatment size effect는 거의 변동 없게끔 만들어서 이해가 될 수 있게.)