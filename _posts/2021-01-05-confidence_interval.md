...
그런데, 실제 모평균의 차이를 우리는 알지 못하므로, 표본 평균에 대해서 수행했던 방법을 그대로 적용하여 신뢰 구간을 표본 평균의 차이를 중심으로 옮겨주면 다음과 같은 결론을 얻을 수 있다.
...
pooled variance는 다음과 같이 계산 가능하며,

$$s^2 = \frac{1}{2} (s_1^2 + s_2^2) = 33.217$$
