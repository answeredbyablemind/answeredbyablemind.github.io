...
여기서 $\vec{b}$, $\vec{c}$ 는 각각 visible layer와 hidden layer의 bias 이다.

어떻게 생각하면 PCA와 비슷한 개념일 수 도 있다. PCA는 고차원의 데이터에 대해 더 저차원의 공간에서의 representation을 찾고자 하는 것이기 때문이다. 하지만 PCA와는 전혀 다른 접근법을 이용한다.
...
$$E(\vec{v}, \vec{h}) = -\vec{b}^T\vec{v} - \vec{c}^T\vec{h} - \vec{v}^TW\vec{h}$$
