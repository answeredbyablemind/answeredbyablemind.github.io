...
주목할 점은 **현재값은 이전 계산결과에서 부터 얻은 값이며, 계속해서 성장량이 누적되어 간다는 점**이다.
...
거기다가 $f(t, x)$에 $x$에 관한 식이 포함되어 있다면 이것은 되먹임(feedback)을 이용한 연속적인 성장이라고 볼 수 있다.
...
다시 말해, 우변에 $x$ 항이 있어야지만 되먹임을 통한 성장이라고 할 수 있다.
...
실제로 식 (1)의 우변에 $x$의 함수가 포함되어 있는 미분방정식을 풀면 그 해가 $e^t$로 표현된다는 것을 알 수 있다.

다시 말해 되먹임을 통한 성장이 바로 자연상수 $e$로 표현되는 것이다. 간단한 예를 들어 아래와 같은 미분 방정식에 대해서,
...
다시 말해, 식 (14)에서 말하고 있는 식은 자율적인 시스템을 구축한 셈이다. 식 (15)는 반대로 외부의 입력을 받아서 시시각각 시스템의 룰이 조금씩 변하고 있는 셈이다.
