...
| --------- |
|    주기 N에 대하여 두 이산 신호 $\phi_k[n]$ 와 $\phi_p [n]$ 는 다음이 성립할 때 직교한다고 할 수 있다.  <br><br>  $$\sum_{n=0}^{N-1}{\phi_k[n]\phi^*_p[n] =0 \space when \space k\neq p }$$. <br><br>여기서 $\phi^*_p[n]$ 에서 '*' 는 켤레복소수 연산임. |
