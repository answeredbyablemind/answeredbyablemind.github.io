...
얼핏 보기에는 1~9까지의 데이터와 19~29까지의 데이터가 구별되어 보이므로 1~9까지의 데이터에 하나의 라벨, 그리고 나머지 데이터에 또 다른 라벨을 주어주면 될 것으로 보인다. 그러면, 우리가 각 라벨의 데이터 별로 평균, 분산 값을 계산할 수 있기 때문이다. 그러면, 우리는 모수값을 알게 된 것이므로 각 라벨 별 분포를 그림 3과 같이 도시할 수 있기 때문이다.

그런데, 반대의 경우를 생각해보자. 만약 그룹이 두 개라고 가정했을 때, 각 그룹 별 평균과 분산 값이 미리 주어져 있었다고 하면 각 그룸에 해당되는 분포의 확률값이 큰 경우에 해당 label을 지정할 수도 있지 않았을까?
