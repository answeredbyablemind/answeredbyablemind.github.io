...
$\phi$는 다시 말하자면 사전 확률과 관련 있는 값이라고 할 수 있다. 

즉, 데이터의 비율만으로 전체 10개의 데이터 중 임의로 선정한 데이터가 0번 그룹에 속하는지 1번 그룹에 속하는지 알고 싶다고 하면 6:4라는 비율을 고려해서 분류할 수 있다.
...
M-step에서는 E-step에서 계산한 $w_j^{(i)}$ 값들을 이용하면 모두 쉽게 계산할 수 있기 때문이다. ($x^{(i)}$는 모두 주어진 데이터라는 점을 꼭 생각하자.)
