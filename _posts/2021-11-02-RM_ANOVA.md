...
One-Way ANOVA에서는 전체 변동(Sum of Squares)이 그룹 간 변동($SS_\text{bet}$)과 그룹 내 변동($SS_\text{wit}$)으로 나눠졌다고 하면

RM ANOVA에서는 전체 변동이 세 가지 변동으로 나누어진다. 피험자 간의 변동, treatment에 의해 생기는 변동, treatment에 반응하는 피험자 내의 변동이다.
