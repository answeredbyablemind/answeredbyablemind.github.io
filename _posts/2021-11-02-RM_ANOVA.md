...
그리고 ANOVA에서는 이 귀무가설을 확인하기 위해 분산을 추정하는데, 각 샘플 집단들이 가지고 있는 분산값을 이용하기도 하고 각 샘플 그룹의 평균값들이 퍼진 정도를 이용해서 분산을 추정한다. 만약 집단 내의 분산에 비해 샘플 집단 평균 간의 분산이 너무 크다면 우리는 귀무가설이 맞기 어려울 것이라고 보고 귀무가설을 기각해 적어도 하나의 샘플 집단은 다른 모집단에서 추출되었을 것이라고 볼 수 있다고 했다.
...
다만, 주어진 데이터 셋은 아래와 같이 표로 정리된 값으로 생각해보도록 하자.
...
$$\sum_s\left(X_{1s}-\bar{X}_1\right)^2$$
