...
for i = unique(round([linspace(x1*0.9, x2*0.9,7),0]))
    ...
    % xticks
    if i<=0
        t= text(i-(y2-y1)*0.05, -(x2-x1)*0.025,num2str(round(i)));
    else
    ...
    % yticks
    if i<0
        t = text(-(y2-y1)*0.1, i, num2str(round(i)));
    elseif i>0
        t = text(-(y2-y1)*0.1, i, num2str(round(i)));
    end
    t.FontSize = fontsize;
...
t = text(x2*1.05,(y2-y1)*0.03,'$$x$$','Interpreter','latex');
t.FontSize = fontsize;
t = text(-(x2-x1)*0.05,y2*1,'$$y$$','Interpreter','latex');
t.FontSize = fontsize;
axis square
